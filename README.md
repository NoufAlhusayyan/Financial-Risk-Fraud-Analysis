# Financial Risk & Fraud Analysis

## Project Overview

This project analyzes financial transactions, customer accounts, and loan data to identify risk patterns and support data-driven financial decisions.

## Objectives

- Analyze transaction patterns and amounts.

- Monitor overdue loans and financial exposure.

- Compare risk across customer and account types.

- Identify unusual transaction patterns.

- Build an interactive Power BI dashboard.

## Tools & Technologies

- Power Query

- MySQL

- SQL

- Power BI

- DAX

- ERD & Data Modeling

## Data Preparation

Power Query was used to clean and prepare the data, including:

- Handling missing values.

- Removing duplicate records.

- Cleaning text fields.

- Handling date errors.

- Reviewing data quality and outliers.

## SQL Analysis

MySQL was used to perform:

- JOIN

- CASE WHEN

- GROUP BY

- HAVING

- VIEW

- Stored Procedure

- Risk-focused analysis

## Power BI Dashboard

### Executive Overview

Provides a high-level view of loans, transactions, overdue exposure, and key financial KPIs.

![Executive Overview](Executive%20Overview.png)

### Customer & Transaction Analysis

Analyzes customer segments, transaction types, transaction amounts, branches, and account types.

![Customer & Transaction Analysis](Customer%20%26%20Transaction%20Analysis.png)

### Risk Analysis

Focuses on overdue loans, overdue exposure, customer types, account types, loan status, and loan amount patterns.

![Risk Analysis](Risk%20Analysis.png)

## SQL Analysis Screenshots

![SQL Analysis 1](SQL1.png)

![SQL Analysis 2](SQL2.png)

## Key Insights

- The overdue loan rate is approximately 10%.

- Overdue loans represent a significant portion of loan exposure.

- Large Enterprise customers have the highest number of overdue loans.

- Deposit, Transfer, and Withdrawal transactions represent the largest transaction activity.

- Average transaction amount is approximately 2,498.55.

- Transaction amount variability was reviewed using standard deviation.

- High account balance alone was not treated as a risk indicator.

## Project Workflow

**Power Query → MySQL → Power BI**

**Power Query:** Data Cleaning  

**MySQL:** SQL Analysis  

**Power BI:** Data Modeling, DAX & Dashboard

## Project Deliverables

- Power BI Dashboard

- SQL Analysis Report

- Project Presentation

- ERD / Data Model

- Data Cleaning
 
